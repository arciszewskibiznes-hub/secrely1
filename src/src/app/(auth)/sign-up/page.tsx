...
export default function SignUpPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { data: authData, error: authError } = await supabase.auth.signUp({
      ...
      password,
      options: {
        // Pass metadata so DB trigger can read it if configured
        data: {
    ...
    // 3. Create profile (upsert is safe — trigger may have already created it)
    ...
    if (profileError) {
      ...
      // Non-fatal: profile may have been created by trigger
    }

    // 4. Seed 500 welcome credits
    ...
    // 5. If email confirmation is required, show a message; otherwise redirect
    if (authData.session) {
