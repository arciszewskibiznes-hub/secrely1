...
import { Coins, TrendingUp, ArrowUpRight, ArrowDownLeft, Gift, Check, Zap, Banknote } from "lucide-react";
...
export default function WalletPage() {
  ...
  const handlePayout = async (e: React.FormEvent) => {
    ...
    if (isNaN(credits) || credits < 100) {
      toast({ title: "Minimum payout is 100 credits", variant: "destructive" });
      return;
    }
    if (balance < credits) {
    ...
    if (!iban || !fullName) {
      toast({ title: "Please fill in all payout fields", variant: "destructive" });
      return;
    }
    ...
    if (!error) {
      // Deduct from balance
      await addCredits(-credits, `Payout request — ${credits} credits → ${amount_pln} PLN`);
      toast({ title: "Payout requested!", description: `${credits} credits → ${amount_pln} PLN will be processed within 3 business days.`, variant: "success" });
      setShowPayout(false);
      setPayoutCredits("");
      setIban("");
      setFullName("");
    ...
    <div className="space-y-6">
      ...
      {showPayout && (
        <motion.div
          ...
          animate={{ opacity: 1, y: 0 }}
          className="card-base p-5 border-purple-200 border-2 space-y-4"
        >
          <div>
            <h3 className="text-sm font-semibold text-foreground">Request a Payout</h3>
            <p className="text-xs text-muted-foreground mt-0.5">1 credit = {CREDIT_TO_PLN} PLN · Minimum 100 credits · Processed in 3 business days</p>
          </div>
          <form onSubmit={handlePayout} className="space-y-3">
            <div className="space-y-1.5">
              <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Credits to withdraw</label>
              <Input
                type="number"
                placeholder="e.g. 500"
                value={payoutCredits}
                ...
                max={balance}
              />
              {payoutCredits && !isNaN(parseInt(payoutCredits, 10)) && (
                <p className="text-xs text-[hsl(270,75%,60%)] font-medium">
                  = {(parseInt(payoutCredits, 10) * CREDIT_TO_PLN).toFixed(2)} PLN
                </p>
              )}
            </div>
            <div className="space-y-1.5">
              <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Full name (as on bank account)</label>
              <Input placeholder="Anna Kowalska" value={fullName} onChange={(e) => setFullName(e.target.value)} />
            </div>
            <div className="space-y-1.5">
      ...
      <div>
        ...
        <p className="text-xs text-muted-foreground text-center mt-3">
          💡 Demo: clicking a package adds credits instantly (no real payment)
        </p>
      </div>
