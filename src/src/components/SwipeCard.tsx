...
import { Lock, Unlock, Heart, Eye, CheckCircle, Coins, Play } from "lucide-react";
...
export function SwipeCard({ post, isTop, stackIndex, isUnlocked, onSwipe, onTap }: SwipeCardProps) {
  ...
  const unlocked = !post.is_locked || isUnlocked;
  ...
  return (
    ...
    >
      <CardInner post={post} unlocked={unlocked} isDragging={isDragging} dragDir={dragDir} skipOpacity={skipOpacity} saveOpacity={saveOpacity} creatorName={creatorName} creatorUsername={creatorUsername} creatorAvatar={creatorAvatar} tags={tags} />
    </motion.div>
  );
}
...
function CardInner({ post, unlocked, skipOpacity, saveOpacity, creatorName, creatorAvatar, tags }: CardInnerProps) {
  return (
        ...
        {post.is_locked ? (
          unlocked ? (
            ...
            <div className="flex items-center gap-1.5 bg-black/50 backdrop-blur-sm text-white rounded-full px-3 py-1 text-xs font-semibold">
              <Coins className="w-3 h-3 text-yellow-300" /> {post.price} credits
            </div>
          )
        ) : (
