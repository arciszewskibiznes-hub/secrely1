...
export function SwipeDeck({ posts, onQueueEmpty }: SwipeDeckProps) {
  ...
  const isOwnPost = (post: Post) => user?.id === ((post as any).creator_id ?? post.creator?.id);
          ...
          {[...visibleCards].reverse().map((post, reversedIdx) => {
            ...
            return (
              <SwipeCard
                ...
                isUnlocked={isUnlocked(post.id) || isOwnPost(post)}
                onSwipe={handleSwipe}
                onTap={() => { if (post.is_locked && !isUnlocked(post.id) && !isOwnPost(post)) setUnlockTarget(post); }}
              />
            );
          })}
        </AnimatePresence>
      ...
      <div className="flex items-center justify-center gap-4 pb-2">
        ...
        {currentPost && (
          <motion.div whileTap={{ scale: 0.93 }} className="flex flex-col items-center gap-1.5">
            {currentPost.is_locked && !isUnlocked(currentPost.id) && !isOwnPost(currentPost) ? (
              <>
                <button
                ...
                <span className="text-[10px] font-semibold text-[hsl(270,75%,60%)] tracking-wide uppercase"><span className="flex items-center gap-1">{t.unlock} <Diamond size={10} className="text-purple-500" />{currentPost.price}</span></span>
              </>
            ) : (
              <>
